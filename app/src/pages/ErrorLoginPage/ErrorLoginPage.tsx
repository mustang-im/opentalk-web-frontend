...
const ErrorLoginPage = ({ error }: ErrorLoginProps) => {
  ...
  const errorDetails = useMemo(() => {
    switch (error.name) {
      ...
      case EventTypeError.OidcConfiguration:
        return {
          name: t('error-oidc-configuration'),
          message: t('error-oidc-configuration-message'),
        };
      case EventTypeError.SystemCurrentlyUnavailable:
        return {
          name: t('error-system-currently-unavailable'),
          message: t('error-system-currently-unavailable'),
        };
      default:
