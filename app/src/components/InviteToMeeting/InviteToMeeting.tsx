  ...
  */
  const fetchPermanentGuestLink = useCallback(async () => {
    ...
    if (invites) {
      ...
      if (foundInvite) {
        return composeInviteUrl(baseUrl, foundInvite.inviteCode, existingEvent.room.id);
      }
    }
    ...
    if (userMe.id === existingEvent.createdBy.id) {
      ...
      return composeInviteUrl(baseUrl, createdInvite.inviteCode, existingEvent.room.id);
    }
  }, []);
